...
use crate::{
    state::{Objective,RoadMapMetaDataStore},
};
...
pub fn handler(ctx: Context<AddChildObjective>,from_root:bool) -> Result<()> {
    ...
    if from_root{
        roadmap_metadata_account.number_of_objectives = roadmap_metadata_account.number_of_objectives.saturating_add(1);
    roadmap_metadata_account.root_objective_ids.push(objective_account.key());
    }else{
        parent_account.children_objective_id.push(objective_account.key());
    }
    Ok(())
}
